...
const { Client } = require('pg'); // For creating database
...
const cred = `postgres://${DB_USER}:${DB_PASSWORD}@${DB_HOST}:${DB_PORT}/${DB_NAME}'`
const sequelize = new Sequelize(cred);

// const sequelize = new Sequelize('postgres://sarniraula:@localhost:5432/fakturera_db');

const Translation = sequelize.define('Translation', {
  key: DataTypes.STRING,
  language: DataTypes.STRING,
  value: DataTypes.TEXT,    //TEXT in PostgreSQL can store very large strings, perfect for our paragraphs.
}, {
...
//Push english language content to translations
enParagraphs.forEach((p, index) => {
  translations.push({ key: `terms_paragraph_${index + 1}`, language: 'en', value: p });
});

//Push sv language content to translations
svParagraphs.forEach((p, index) => {
  translations.push({ key: `terms_paragraph_${index + 1}`, language: 'sv', value: p });
});
...
async function insertData() {
  try {
    ...
    await sequelize.sync({ force: true });    // force:true drops & recreates table
